...
START
 ...
 |   LetsEnc            Use the le.pl/Crypt::LE (ver. >21) to query for/request renewal [ quit here if cert doesnt need renewing ]
...
sub LetsEnc {
        if ((qx(which le.pl)) ne 0) { die "Not found\n"; }
        ...
        if ( $? eq 88) {
                ...
                die "Renewal not called\n";
        }
}

sub AppendRC {
        open ( FH0, ">>", "$CertNameTMP" ) || die "Couldn't open $CertNameTMP\n";
        open ( FH1, "<", "$RootCert" ) || die "Couldn't open $RootCert\n";
        while ( my $readline = <FH1> ) { print FH0 $readline; }
